...
void foo() {
	...
	PTHREAD_TRACE_NP(Qp0zUprintf("foo(), threadSpecific data=%d %d\n", gData->threadSpecific1,
		gData->threadSpecific2);, PTHREAD_TRACE_INFO_NP);
...
int main(int argc, char **argv) {
	...
	for (i=0; i <NUMTHREADS; ++i) {
		...
		PTHREAD_TRACE_NP(Qp0zUprintf("Wait for the thread to complete, and release their resources\n");,
			PTHREAD_TRACE_INFO_NP);
