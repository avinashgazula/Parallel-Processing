...
#define NELTS 500000     /* number of elements in the vector */
...
/******* master process ***********/
	if (rank == MASTER) {
		...
		printf("Initial distribution took %lf seconds.\n",end-start);
		...
		printf("Time spent waiting for slaves was %lf seconds.\n",end-start);
...
/************ worker processes *************/
 	if (rank > MASTER) {
		...
		if (rank==1)  // have first slave track compute time
			start=MPI_Wtime();
		...
		if (rank==1) { // have first slave report compute time
			end=MPI_Wtime();
			printf("Time for computation took %lf seconds.\n",end-start);
		}
